...
For example we might want to specify different database hostname in certain cases:
...
<2> Property `postgres.host` is equals to `prosgres` when application is running with staging profile `mvn quarkus:dev -Dquarkus.profile=staging`.
...
Compile or run Quarkus application with proper profile according to environment:

[source,shell]
----
./mvnw package -Pnative -Dquarkus.profile=staging
